...
type QuSpan map[string]interface{}

func NewQuSpan() QuSpan {
	return make(QuSpan)
}

func (s QuSpan) Set(key string, value interface{}) {
	s[key] = value
}

func (s QuSpan) SetTraceID(traceIDHigh, traceIDLow int64) {
	var traceID string
	if traceIDHigh != 0 {
...
func (s QuSpan) SetParentSpanID(parentSpanID int64) {
	if parentSpanID != 0 {
...
func (s QuSpan) SetSpanID(spanID int64) {
	s.Set("spanID", fmt.Sprintf("%016x", uint64(spanID)))
}

func (s QuSpan) SetOperationName(operationName string) {
	s.Set("operationName", operationName)
}

func (s QuSpan) SetTag(key string, value string) {
	s.Set("tags."+key, value)
}

func (s QuSpan) SetServiceName(serviceName string) {
	s.Set("process.serviceName", serviceName)
}

func (s QuSpan) SetHostname(hostname string) {
	s.Set("process.tags.hostname", hostname)
}

func (s QuSpan) SetIP(ip string) {
	s.Set("process.tags.ip", ip)
}

func BuildQuSpan(span *jaeger.Span) QuSpan {
