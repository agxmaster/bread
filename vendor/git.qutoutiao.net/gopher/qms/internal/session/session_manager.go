...
func SaveSessionIDFromHTTP(ep string, autoTimeout int, resp *http.Response, req *http.Request) {
	...
	valueQmsLb := GetSessionFromResp(common.LBSessionID, resp)
	//if session is in resp, then just save it
	if valueQmsLb != "" {
		Save(valueQmsLb, ep, timeValue)
...
func DeletingKeySuccessiveFailure(resp *http.Response) {
	Cache.DeleteExpired()
	if resp == nil {
		valueQmsLb := getLBCookie(common.LBSessionID)
		if valueQmsLb != "" {
			cookieKey := strings.Split(valueQmsLb, "=")
			if len(cookieKey) > 1 {
				Delete(cookieKey[1])
