...
func (qpsL *LimiterMap) GetUpstreamQPSRate(keys *ConsumerKeys) (int, string) {
	// api > service
	limit := config.GetUpstream(keys.MicroServiceName).RateLimit.Limit
	if qps, ok := limit["limit."+strings.ToLower(keys.OperationID)]; ok {
		return qps, strings.Join([]string{common.Consumer, keys.MicroServiceName, keys.OperationID}, ".") // 返回的key是给limiterMap做缓存用的
	}
	return limit["limit"], strings.Join([]string{common.Consumer, keys.MicroServiceName}, ".")
}

func (qpsL *LimiterMap) GetServiceQPSRate(keys *ProviderKeys) (int, string) {
	// api > service
	limit := config.GetService(keys.MicroServiceName).RateLimit.Limit
	if qps, ok := limit["limit."+strings.ToLower(keys.OperationID)]; ok {
		return qps, strings.Join([]string{common.Provider, keys.MicroServiceName, keys.OperationID}, ".") // 返回的key是给limiterMap做缓存用的
	}
	return limit["limit"], strings.Join([]string{common.Provider, keys.MicroServiceName}, ".")
}
