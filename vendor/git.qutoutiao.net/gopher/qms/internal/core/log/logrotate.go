...
// Rotate function for log rotate
...
func Rotate(path string, MaxFileSize int, MaxBackupCount int) {
	...
	defer func() {
		if e := recover(); e != nil {
			qlog.Errorf("Rotate catch an exception")
		}
	}()
...
func initLogRotate(logFilePath string, option *Options) {
	...
	if option.RollingPolicy == RollingPolicySize {
		go func() {
			for {
				Rotate(filepath.Dir(logFilePath), option.LogRotateSize, option.LogBackupCount)
		...
		go func() {
			for {
				Rotate(filepath.Dir(logFilePath), 0, option.LogBackupCount)
