...
type CseStruct struct {
	...
	Transport       Transport                `yaml:"transport"` //Deprecated
	Tracing         TracingStruct            `yaml:"tracing"`
	NoRefreshSchema bool                     `yaml:"noRefreshSchema"`
	AccessLog       AccessLog                `yaml:"accessLog"`
	PProf           PProf                    `yaml:"pprof"`
	Swagger         Swagger                  `yaml:"swagger"` //Deprecated
	CustomRoute     map[string][]interface{} `yaml:"customRoute"`
}
...
type Transport struct {
	Failure         map[string]string `yaml:"failure"`      //Deprecated
	MaxIdlConsMap   map[string]int    `yaml:"maxIdleCon"`   //Deprecated
	MaxBodyBytesMap map[string]int64  `yaml:"maxBodyBytes"` //Deprecated
}
