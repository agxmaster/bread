...
func GetFallbackEnabled(command, t string) bool {
	return qconf.GetBool(GetFallbackEnabledKey(command),
		qconf.GetBool(GetDefaultGetFallbackEnabledKey(t), DefaultFallbackEnable))
}

// GetCircuitBreakerEnabled get circuit breaker enabled
func GetCircuitBreakerEnabled(command, t string) bool { // 只有provider使用
	return qconf.GetBool(GetCircuitBreakerEnabledKey(command),
		qconf.GetBool(GetDefaultCircuitBreakerEnabledKey(t), DefaultProviderCircuitBreakerEnabled))
}
...
func GetForceClose(command, t string) bool {
	...
	global := cbspec.ForceClose
	ret := qconf.GetBool(GetForceCloseKey(command), global)
	cbMutex.RUnlock()
	return ret
}
...
func GetForceOpen(command, t string) bool {
	cbMutex.RLock()
	cbspec := getCircuitBreakerSpec(t)
	global := cbspec.ForceOpen
	ret := qconf.GetBool(GetForceOpenKey(command), global)
	cbMutex.RUnlock()
	return ret
}
...
func GetTimeout(service, t string) int {
	...
	m := qconf.GetInt(GetTimeoutKey(service), global)
	cbMutex.RUnlock()
	return m
}
...
func GetMaxConcurrentRequests(command, t string) int {
	...
	m := qconf.GetInt(GetMaxConcurrentKey(command), global)
	cbMutex.RUnlock()
	return m
}
...
func GetErrorPercentThreshold(command, t string) int {
	...
	m := qconf.GetInt(GetErrorPercentThresholdKey(command), global)
	cbMutex.RUnlock()
	return m
}
...
func GetRequestVolumeThreshold(command, t string) int {
	...
	m := qconf.GetInt(GetRequestVolumeThresholdKey(command), global)
	cbMutex.RUnlock()
	return m
}
...
func GetSleepWindow(command, t string) int {
	...
	m := qconf.GetInt(GetSleepWindowKey(command), global)
	cbMutex.RUnlock()
	return m
}
