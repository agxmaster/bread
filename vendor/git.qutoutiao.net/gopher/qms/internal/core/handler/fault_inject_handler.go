...
import (
	...
	"git.qutoutiao.net/gopher/qms/internal/core/fault"
...
func (rl *FaultHandler) Handle(chain *Chain, inv *invocation.Invocation, cb invocation.ResponseCallBack) {
	faultConfig := GetFaultConfig(inv.Protocol.String(), inv.MicroServiceName, inv.SchemaID, inv.OperationID)

	faultInject, ok := fault.Injectors[inv.Protocol.String()]
	r := &invocation.Response{}
	if !ok {
		msg := "fault injection doesn't support for protocol " + inv.Protocol.String()
		qlog.Error(msg)
