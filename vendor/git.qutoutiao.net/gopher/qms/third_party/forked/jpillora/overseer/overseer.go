...
import (
	...
	"git.qutoutiao.net/gopher/qms/third_party/forked/jpillora/overseer/fetcher"
)

const (
	masterVersion = "v1.0.0"

	envMasterVersion  = "OVERSEER_MASTER_VERSION"
	envAddresses      = "OVERSEER_ADDRESSES"
	envRestartPort    = "OVERSEER_RESTART_PORT"
...
type Config struct {
	...
	RestartPort int
	// RestartTimeout 服务启动需要的时间(用户侧)，默认8s
...
func validate(c *Config) error {
	...
	if c.Logger == nil {
		return errors.New("overseer.Config.Logger required")
	}
	if c.Address != "" {
	...
	if c.RestartTimeout <= 0 {
		c.RestartTimeout = 8 * time.Second
	}
	return nil
}
...
func runErr(c *Config) error {
	...
	if os.Getenv(envIsSlave) == "1" {
		oldVersion := os.Getenv(envMasterVersion)
		newVersion := masterVersion
		if newVersion != oldVersion {
			//注：以下当log信息将作为发版时当判断，请不要随意改动内容。
			c.Logger.Warnf("QMS-MASTER-INFO-CHANGED-%d: version from %s to %s", os.Getpid(), oldVersion, newVersion)
		}

		oldAddrs := os.Getenv(envAddresses)
		newAddrs := strings.Join(c.Addresses, "-")
		if newAddrs != oldAddrs {
			//注：以下当log信息将作为发版时当判断，请不要随意改动内容。
			c.Logger.Warnf("QMS-MASTER-INFO-CHANGED-%d: addresses from %s to %s", os.Getpid(), oldAddrs, newAddrs)
		}

		oldPort := os.Getenv(envRestartPort)
		newPort := strconv.Itoa(c.RestartPort)
		if oldPort != newPort {
			//注：以下当log信息将作为发版时当判断，请不要随意改动内容。
			c.Logger.Warnf("QMS-MASTER-INFO-CHANGED-%d: restart port from %s to %s", os.Getpid(), oldPort, newPort)
		}

		currentProcess = newSlave(c)
	} else {
		currentProcess = newMaster(c)
	}
